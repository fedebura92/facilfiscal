...
function Choice({active,onClick,children}:{active:boolean;onClick:()=>void;children:ReactNode}){return <button type="button" onClick={onClick} style={{minHeight:50,padding:'11px 13px',borderRadius:9,border:`2px solid ${active?'#0d9488':'#e2e8f0'}`,background:active?'#ecfdf5':'white',fontFamily:'inherit',fontSize:15,lineHeight:1.35,fontWeight:600,cursor:'pointer'}}>{children}</button>}
...
export function ProvinceCalculatorPage(){
 ...
 return <main style={{background:'#f8fafc',minHeight:'100vh'}}>
  <header style={{background:'linear-gradient(135deg,#1d4ed8,#312e81)',color:'white',padding:'52px 20px',textAlign:'center'}}><h1 style={{fontSize:'clamp(30px,5vw,46px)',lineHeight:1.15,letterSpacing:'-0.02em',margin:'0 0 12px',fontWeight:900}}>Calculadora de impuestos por provincia</h1><p style={{maxWidth:730,margin:'auto',fontSize:18}}>Elegí tu provincia y actividad. Calculá Ingresos Brutos y consultá el organismo que te corresponde.</p></header>
