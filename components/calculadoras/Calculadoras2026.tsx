...
const question:CSSProperties={margin:'0 0 10px',fontSize:'clamp(19px,2.2vw,22px)',lineHeight:1.3,fontWeight:900,letterSpacing:'-0.01em',color:'#102a36'}
...
function Choice({active,onClick,children}:{active:boolean;onClick:()=>void;children:ReactNode}){return <button type="button" onClick={onClick} style={{minHeight:50,padding:'11px 13px',borderRadius:9,border:`2px solid ${active?'#0d9488':'#e2e8f0'}`,background:active?'#ecfdf5':'white',fontFamily:'inherit',fontSize:15,lineHeight:1.35,fontWeight:800,cursor:'pointer'}}>{children}</button>}
...
export function GananciasCalculator(){
 ...
 return <Shell title="Calculadora de Ganancias 2026" intro="Decinos cuánto ganás y respondé preguntas simples. Nosotros hacemos las deducciones." color="linear-gradient(135deg,#7c3aed,#4c1d95)" source="https://www.arca.gob.ar/gananciasYBienes/ganancias/personas-humanas-sucesiones-indivisas/deducciones/deducciones-personales.asp">
  ...
  <section style={formSection}><h2 style={question}>2. ¿Cuánto cobrás normalmente?</h2><label style={fieldLabel}><strong>{tipo==='empleado'?'Sueldo bruto mensual (antes de descuentos)':'Ingresos o facturación mensual'}</strong><input style={input} type="number" min="0" value={ing} onChange={e=>setIng(e.target.value)} placeholder="Ej.: 2500000"/></label>
