...
const CapturaEmail=forwardRef<HTMLInputElement,{tipoDefault:'mono'|'ri'|'aut'}>(function CapturaEmail({tipoDefault},ref){
  const[email,setEmail]=useState(''),[ok,setOk]=useState(false),[bienvenida,setBienvenida]=useState(true),[tipos,setTipos]=useState<string[]>([tipoDefault]),[terminacion,setTerminacion]=useState(''),[dias,setDias]=useState(3),[error,setError]=useState(''),[loading,setLoading]=useState(false)
  ...
  async function submit(){
    ...
    if(requiereCuit&&!/^[0-9]$/.test(terminacion))return setError('Elegí el último número de tu CUIT')
    setLoading(true);setError('')
    try{
      const r=await fetch('/api/suscribir',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email.trim(),tipos,terminacionCuit:terminacion||null,diasAnticipacion:dias})})
      const d=await r.json()
      if(!r.ok)return setError(d.error||'Error al guardar')
      setBienvenida(d.bienvenidaEnviada!==false)
      setOk(true)
  ...
  return <div className="ff-captura" style={{background:'linear-gradient(135deg,var(--teal-dark),var(--teal))',position:'relative',overflow:'hidden',boxShadow:'var(--sh-lg)'}}>
    ...
    <div className="ff-cap-form" style={{position:'relative',zIndex:1}}>
      {ok?<div><div style={{color:'var(--gold)',fontSize:15,fontWeight:800}}>✓ Alertas activadas.</div><div style={{fontSize:12,color:'rgba(255,255,255,.8)',fontWeight:600,marginTop:4}}>{bienvenida?'Te enviamos un email de confirmación.':'La suscripción quedó guardada, aunque el email de bienvenida no pudo enviarse.'}</div></div>:<>
        ...
        <div className="ff-cap-input-row"><input ref={ref} type="email" placeholder="tu@email.com" value={email} onChange={e=>{setEmail(e.target.value);setError('')}} onKeyDown={e=>e.key==='Enter'&&!loading&&submit()} style={{...inputStyle,flex:1,minWidth:0}}/><button onClick={submit} disabled={loading} style={{background:'var(--gold)',color:'var(--ink)',border:'none',borderRadius:8,padding:'10px 16px',fontSize:13,fontWeight:900,whiteSpace:'nowrap',cursor:loading?'not-allowed':'pointer',opacity:loading?.7:1}}>{loading?'Guardando…':'Activar →'}</button></div>
      </>}
    </div>
  </div>
})
