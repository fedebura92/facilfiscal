    ...
    <>
      ...
      <header style={{background:V.surface,borderBottom:`1px solid ${V.border}`,position:'sticky',top:0,zIndex:100,boxShadow:`0 1px 4px rgba(13,92,120,.07)`}}>
        <div style={{maxWidth:860,margin:'0 auto',padding:'0 24px',height:56,display:'flex',alignItems:'center',gap:16}}>
          ...
          <span style={{fontSize:13,fontWeight:700,color:V.ink2}}>Mi categoría</span>
        </div>
      </header>
      ...
      <main style={{maxWidth:860,margin:'0 auto',padding:'32px 24px 80px'}}>
        ...
        <div style={{background:`linear-gradient(135deg,${V.tealDark},${V.teal})`,borderRadius:16,padding:'28px 32px',marginBottom:28,color:'white'}}>
          <div style={{fontSize:11,fontWeight:800,letterSpacing:'2px',textTransform:'uppercase',color:'rgba(255,255,255,.6)',marginBottom:6}}>Monotributo 2026</div>
          <div style={{fontSize:26,fontWeight:900,letterSpacing:'-0.3px',marginBottom:6}}>Calculadora de categoría</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,.7)'}}>Ingresá tu facturación mensual y te decimos qué categoría te corresponde</div>
        </div>
        ...
        <div style={{background:V.surface,border:`1.5px solid ${V.border}`,borderRadius:16,overflow:'hidden',marginBottom:24,boxShadow:`0 1px 4px rgba(13,92,120,.07)`}}>
          ...
          <div style={{padding:'20px 18px'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:16}}>
              <div>
                <div style={{fontSize:11,fontWeight:800,letterSpacing:'0.8px',textTransform:'uppercase',color:V.ink3,marginBottom:6}}>
                ...
                <div style={{display:'flex',alignItems:'center',border:`1.5px solid ${V.border}`,borderRadius:8,overflow:'hidden',background:V.bg}}>
                  ...
                  <input
                    ...
                    style={{flex:1,border:'none',padding:'10px 12px',fontSize:14,fontWeight:600,color:V.ink,background:'transparent',outline:'none'}}
                  />
                </div>
              </div>
              <div>
                ...
                <select value={conOS?'si':'no'} onChange={e=>setConOS(e.target.value==='si')}
                  style={{width:'100%',border:`1.5px solid ${V.border}`,borderRadius:8,padding:'10px 12px',fontSize:14,fontWeight:600,color:V.ink,background:V.bg,outline:'none'}}>
            ...
            {/* RESULTADO */}
            {excede && (
              <div style={{marginTop:16,background:V.redBg,border:`1.5px solid ${V.redRing}`,borderRadius:10,padding:'16px 18px',textAlign:'center'}}>
                <div style={{fontSize:16,fontWeight:900,color:V.red,marginBottom:4}}>⚠️ Superás el límite del Monotributo</div>
                <div style={{fontSize:13,color:'#7a2020',fontWeight:600}}>Tu facturación anual supera los $72.000.000. Debés pasar al régimen general como Responsable Inscripto.</div>
              </div>
            )}

            {resultado && (
              <div style={{marginTop:16,background:V.greenBg,border:`1.5px solid ${V.greenRing}`,borderRadius:10,padding:'18px'}}>
                <div style={{textAlign:'center',marginBottom:14}}>
                  <div style={{fontSize:13,color:V.green,fontWeight:700,marginBottom:4}}>Tu categoría es</div>
                  <div style={{fontSize:52,fontWeight:900,color:V.green,lineHeight:1}}>Categoría {resultado.letra}</div>
                  <div style={{fontSize:13,color:'#166534',fontWeight:600,marginTop:4}}>Límite anual: {money(resultado.limite)}</div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                  {[
                    ['Impositivo', money(resultado.imp)],
                    ['Previsional', money(resultado.prev)],
                    ['Obra social', conOS ? money(OS_EXTRA) : 'No incluida'],
                    ['Total mensual', money(resultado.imp + resultado.prev + (conOS?OS_EXTRA:0))],
                  ].map(([l,v],i)=>(
                    <div key={l} style={{background:'rgba(255,255,255,.7)',borderRadius:8,padding:'10px 12px',textAlign:'center',gridColumn:i===3?'1/-1':'auto'}}>
                      <div style={{fontSize:10,fontWeight:800,textTransform:'uppercase',color:'#166534',letterSpacing:'0.5px'}}>{l}</div>
                      <div style={{fontSize:i===3?20:15,fontWeight:900,color:V.green,marginTop:3}}>{v}</div>
                    </div>
                  ))}
                </div>
        ...
        <div style={{background:V.surface,border:`1.5px solid ${V.border}`,borderRadius:16,overflow:'hidden',marginBottom:24,boxShadow:`0 1px 4px rgba(13,92,120,.07)`}}>
          <div style={{padding:'14px 18px',borderBottom:`1px solid ${V.border}`,fontSize:14,fontWeight:800,color:V.ink}}>
            📊 Tabla completa de categorías 2026
          </div>
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
              ...
              <tbody>
                {CATEGORIAS.map((c,i)=>(
                  <tr key={c.letra} style={{background:i%2===0?V.surface:V.bg}}>
                    <td style={{padding:'10px 14px',fontWeight:900,fontSize:15,color:V.teal}}>{c.letra}</td>
          ...
          <div style={{padding:'10px 18px',borderTop:`1px solid ${V.border}`,fontSize:11,color:V.ink3,fontWeight:600}}>
            * Valores estimados 2026. Se actualizan periódicamente por resolución de ARCA.
          </div>
        </div>
          ...
          <div style={{padding:'14px 16px'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {[
                ...
                '¿Me conviene pasar a RI?',
                '¿Cuáles son los valores actualizados 2026?',
              ].map(c=>(
