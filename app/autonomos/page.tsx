      ...
      <div className="ff-page-content">
        ...
        <div className="ff-hero" style={{ background:`linear-gradient(135deg,${V.tealDark} 0%,${V.teal} 100%)`, position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ maxWidth:640, margin:'0 auto', position:'relative', zIndex:1 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(255,255,255,.12)', border:'1px solid rgba(255,255,255,.2)', borderRadius:20, padding:'5px 14px', marginBottom:16, fontSize:12, fontWeight:700, color:'rgba(255,255,255,.9)' }}>
              <span>⚡</span> Para trabajadores autónomos de Argentina
            </div>
            <h1>Gestioná tus aportes<br/>como autónomo sin errores</h1>
            <p>Controlá tus vencimientos de aportes jubilatorios,<br/>IVA y Ganancias. Recibí recordatorios automáticos.</p>
        ...
        <div style={{ background:V.surface, borderBottom:`1px solid ${V.border}` }}>
          <div style={{ maxWidth:860, margin:'0 auto', padding:'20px 16px' }}>
            <div style={{ textAlign:'center', marginBottom:14 }}>
              ...
              <div style={{ fontSize:16, fontWeight:900, color:V.ink }}>Errores comunes del trabajador autónomo</div>
            </div>
            <div className="ff-dolor-grid">
              {[
                { icon:'❌', txt:'Pagan aportes en categoría incorrecta' },
                { icon:'❌', txt:'No saben si tienen que pagar IVA' },
                { icon:'❌', txt:'Olvidan los aportes jubilatorios' },
        ...
        <main className="ff-main">
          ...
          <div className="ff-two-col">
            ...
            <div style={{ background:V.surface, border:`1.5px solid ${V.border}`, borderRadius:14, overflow:'hidden', boxShadow:`0 1px 4px rgba(13,92,120,.07)` }}>
              <div style={{ padding:'13px 16px', borderBottom:`1px solid ${V.border}`, fontSize:14, fontWeight:800, color:V.ink }}>🧮 Estimación de aportes</div>
              <div style={{ padding:'14px 16px' }}>
                ...
                <select value={catIdx} onChange={e => setCatIdx(e.target.value)} style={{ width:'100%', border:`1.5px solid ${V.border}`, borderRadius:8, padding:'9px 10px', fontSize:13, fontWeight:600, color:V.ink, background:V.bg, outline:'none', marginBottom:10 }}>
                  ...
                  {MONTOS[TIPO].cats.map((c, i) => <option key={i} value={i}>{c} — {MONTOS[TIPO].limites[i]}</option>)}
                </select>
                <div style={{ background:`linear-gradient(135deg,${V.tealDark},${V.tealMid})`, borderRadius:8, padding:'16px', textAlign:'center', minHeight:80, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
                  {catIdx !== '' ? <>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, width:'100%' }}>
                      {([['Jubilatorio', MONTOS[TIPO].imp[idx]], ['Previsional', MONTOS[TIPO].prev[idx]]] as [string, number][]).map(([l, val]) => (
          ...
          <div style={{ background:V.surface, border:`1.5px solid ${V.border}`, borderRadius:14, overflow:'hidden', marginBottom:20, boxShadow:`0 1px 4px rgba(13,92,120,.07)` }}>
            ...
            <div style={{ padding:'12px 14px' }}>
              <div style={{ display:'flex', gap:8, marginBottom:10 }}>
                <input value={aiQuery} onChange={e => setAiQuery(e.target.value)} onKeyDown={e => e.key==='Enter'&&askAI()} placeholder="¿Cuánto pago como autónomo categoría III?"
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                {['Cuándo vencen los aportes','Categorías autónomos','Inscripción en IVA','Diferencia con monotributo'].map(c => (
          ...
          {/* CAPTURA */}
          <CapturaEmail ref={capturaRef} tipoDefault="aut" />
        </main>
      </div>
