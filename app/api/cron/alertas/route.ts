...
async function enviarAlerta(a: Alerta): Promise<string|null> {
  ...
  const html = `<!doctype html><html lang="es"><body style="font-family:Arial,sans-serif;background:#f4f7f9;padding:32px 16px"><main style="max-width:520px;margin:auto;background:white;border-radius:16px;overflow:hidden"><header style="background:#0d5c78;padding:20px 28px 22px;color:white;text-align:center"><div style="display:inline-block;background:white;border-radius:10px;padding:8px 14px;margin-bottom:10px"><img src="${LOGO_URL}" alt="Fácil Fiscal" width="180" style="display:block;width:180px;max-width:100%;height:auto;border:0"/></div><div style="font-size:12px;color:rgba(255,255,255,.82)">Recordatorio de vencimiento</div></header><section style="padding:28px"><p>Hola ${escaparHtml(a.nombre)},</p><div style="background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:18px"><b style="color:#d97706;font-size:18px">${urgencia}</b><h2>${escaparHtml(a.titulo)}</h2><p>Fecha: ${fecha}</p></div>${fuente}<a href="${BASE_URL}/mipanel" style="display:block;background:#1a7fa8;color:white;text-align:center;padding:14px;border-radius:10px;text-decoration:none">Ver mis vencimientos →</a></section><footer style="background:#f4f7f9;padding:16px;text-align:center;font-size:11px"><a href="${BASE_URL}/unsubscribe?email=${encodeURIComponent(a.to)}&token=${generarTokenUnsub(a.to)}">Cancelar suscripción</a></footer></main></body></html>`
