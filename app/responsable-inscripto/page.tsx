...
const MONTOS = {
  ...
  ri:{cats:['Pequeño RI (hasta $30M)','Mediano RI (hasta $100M)','Gran RI (más de $100M)'],limites:['hasta $30M/año','hasta $100M/año','más de $100M/año'],imp:[18000,35000,60000],prev:[28000,40000,50000],os:0},
  aut:{cats:['Categoría I','Categoría II','Categoría III','Categoría IV','Categoría V'],limites:['menor actividad','—','—','—','mayor actividad'],imp:[15000,22000,32000,48000,65000],prev:[22000,28000,35000,45000,58000],os:0},
}
...
export default function ResponsableInscriptoPage() {
  ...
  const vencOrd = useMemo(()=>[...venc].filter(v=>diff(v.fecha)>=0).sort((a,b)=>diff(a.fecha)-diff(b.fecha)),[venc])
  const cards = useMemo(()=>vencOrd.slice(0,3),[vencOrd])
  const proximos = useMemo(()=>vencOrd.filter(v=>diff(v.fecha)<=10).slice(0,8),[vencOrd])
  const idx = useMemo(()=>parseInt(catIdx),[catIdx])
  const total = useMemo(()=>{if(catIdx==='')return 0;return MONTOS[tipo].imp[idx]+MONTOS[tipo].prev[idx]},[catIdx,idx])
    ...
    <>
      <style dangerouslySetInnerHTML={{__html:`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .ff-calc-break{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}
      ...
      <div style={{background:V.surface,borderBottom:`1px solid ${V.border}`}}>
        <div style={{maxWidth:1120,margin:'0 auto',padding:'24px 16px'}}>
          ...
          <div style={{textAlign:'center',marginTop:14}}>
            <a href="https://www.afip.gob.ar" target="_blank" rel="noopener noreferrer" style={{fontSize:13,fontWeight:800,color:V.teal,textDecoration:'none'}}>Fácil Fiscal te ayuda a evitar todo esto →</a>
          </div>
        </div>
      </div>
